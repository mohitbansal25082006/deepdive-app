...
function SkeletonCard({ index }: { index: number }) {
  ...
  return (
    <View style={{
      borderRadius: RADIUS.xl, height: 134, marginBottom: SPACING.sm, overflow: 'hidden',
...
function StatCard({ icon, value, label, gradient, accent }: {
  ...
  return (
    <View style={{ flex: 1, borderRadius: RADIUS.lg, overflow: 'hidden', borderWidth: 1, borderColor: `${accent}30` }}>
      <LinearGradient colors={['#1A1A38', '#12122A']} style={{ paddingVertical: SPACING.sm, paddingHorizontal: 6, alignItems: 'center', gap: 5 }}>
        <LinearGradient colors={gradient} style={{ width: 32, height: 32, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }}>
          <Ionicons name={icon as any} size={15} color="#FFF" />
        </LinearGradient>
        <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.md, fontWeight: '900', lineHeight: 21 }} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.7}>{value}</Text>
        <Text style={{ color: COLORS.textMuted, fontSize: 10, fontWeight: '700', textAlign: 'center' }} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}>{label}</Text>
      </LinearGradient>
    </View>
  );
}
...
function BookmarkCard({
  ...
  const handleRemove = () => {
    bm.value = withSequence(withSpring(1.3, { damping: 5, stiffness: 260 }), withSpring(1, { damping: 12 }));
  ...
  const chips: { icon: string; label: string; color: string; bg: string }[] = [
    ...
    ...((report.citations?.length ?? 0) > 0 ? [{ icon: 'link-outline', label: `${report.citations.length} sources`, color: COLORS.textSecondary, bg: 'rgba(255,255,255,0.05)' }] : []),
  ...
  return (
    ...
    >
      ...
      >
        <View style={{ borderRadius: RADIUS.xl, overflow: 'hidden', borderWidth: 1, borderColor: `${COLORS.primary}40` }}>
          <LinearGradient colors={['#16162F', '#101024']} style={{ padding: SPACING.md, paddingLeft: SPACING.md + 6 }}>
            {/* depth accent rail */}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: SPACING.sm }}>
              ...
              <Animated.View style={bmStyle}>
                <Pressable onPress={handleRemove} hitSlop={{ top: 8, right: 8, bottom: 8, left: 8 }} style={{ width: 32, height: 32, borderRadius: 9, backgroundColor: `${COLORS.primary}1A`, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: `${COLORS.primary}40` }}>
  ...
  return (
    <LinearGradient colors={[COLORS.background, '#0B0B1E', COLORS.backgroundCard]} style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <Animated.View entering={FadeIn.duration(500)}>
          <LinearGradient colors={['#15152F', '#0D0D22']} style={{ paddingBottom: SPACING.sm }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.xl, paddingTop: SPACING.md, paddingBottom: SPACING.sm, gap: SPACING.md }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.xl, fontWeight: '900', lineHeight: 28, letterSpacing: -0.4 }}>Saved Reports</Text>
              ...
              <LinearGradient colors={COLORS.gradientPrimary} style={{ width: 44, height: 44, borderRadius: 14, alignItems: 'center', justifyContent: 'center', ...SHADOWS.small }}>
            ...
            {reports.length > 0 && (
              <View style={{ flexDirection: 'row', gap: SPACING.sm, paddingHorizontal: SPACING.xl, marginBottom: SPACING.sm }}>
                <StatCard icon="bookmark" value={String(reports.length)} label="Saved" gradient={COLORS.gradientPrimary} accent={COLORS.primary} />
                <StatCard icon="shield-checkmark" value={avgScore ? `${avgScore}` : '—'} label="Avg Score" gradient={[COLORS.success, '#38F9D7']} accent={COLORS.success} />
                <StatCard icon="globe" value={totalSources >= 1000 ? `${(totalSources / 1000).toFixed(1)}k` : String(totalSources)} label="Sources" gradient={[COLORS.info, '#4FACFE']} accent={COLORS.info} />
              </View>
            )}
          </LinearGradient>
        </Animated.View>
        ...
        {/* ── Content ── */}
        <ScrollView
          contentContainerStyle={{ paddingHorizontal: SPACING.xl, paddingTop: SPACING.md, paddingBottom: 110 }}
        ...
        >
          ...
          {!loading && reports.length === 0 && (
            <Animated.View entering={FadeIn.duration(600)} style={{ alignItems: 'center', paddingTop: 70 }}>
              <LinearGradient colors={['#1C1C40', '#14142E']} style={{ width: 100, height: 100, borderRadius: 30, alignItems: 'center', justifyContent: 'center', marginBottom: SPACING.lg, borderWidth: 1, borderColor: `${COLORS.primary}30` }}>
              ...
              <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.sm, textAlign: 'center', lineHeight: 22, paddingHorizontal: SPACING.xl, marginBottom: SPACING.xl }}>
                Tap the bookmark icon on any research report to save it here for quick access.
              </Text>

              <Pressable onPress={() => { haptic(); router.push('/(app)/(tabs)/history'); }}>
                <LinearGradient colors={COLORS.gradientPrimary} style={{ borderRadius: RADIUS.full, paddingHorizontal: SPACING.xl, paddingVertical: 13, flexDirection: 'row', alignItems: 'center', gap: 8, ...SHADOWS.medium }}>
                  ...
                  <Text style={{ color: '#FFF', fontWeight: '800', fontSize: FONTS.sizes.base }}>Browse History</Text>
                </LinearGradient>
              </Pressable>
            </Animated.View>
          )}
          ...
          {reports.length > 0 && (
            <>
              <Animated.View
                ...
                style={{
                  backgroundColor: `${COLORS.primary}08`, borderRadius: RADIUS.lg, padding: SPACING.sm,
                  marginBottom: SPACING.md, flexDirection: 'row', alignItems: 'center', gap: 8,
