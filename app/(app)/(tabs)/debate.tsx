...
// Part 40 Fix — UPDATED: Auto-fills topic input from report.
// Part 55.2 — FULL THEME-COMPATIBILITY PASS + Date overflow fix
...
import { COLORS, FONTS, SPACING, RADIUS, SHADOWS, getAuroraGradient } from '../../../src/constants/theme';
...
function DebateHistoryCard({
  session, index, onPress, onDelete, onLongPress,
}: {
  session: DebateSession; index: number;
  ...
  return (
    <Animated.View entering={FadeInDown.duration(350).delay(index * 60)}>
      <TouchableOpacity
        ...
        onLongPress={isCompleted ? onLongPress : undefined}
        activeOpacity={isCompleted ? 0.85 : 1}
        style={{
          backgroundColor: COLORS.backgroundCard, borderRadius: RADIUS.xl, marginBottom: SPACING.md,
          borderWidth: 1, borderColor: isFailed ? `${COLORS.error}30` : COLORS.border,
          ...
          <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10, marginBottom: SPACING.sm }}>
            <View style={{
              ...
              <Ionicons
                ...
                size={18} color={isFailed ? COLORS.error : COLORS.primary}
              />
            </View>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.sm, marginBottom: SPACING.sm }}>
              <View style={{ flexDirection: 'row' }}>
                {agentColors.slice(0, 6).map((color, i) => (
                  <View key={i} style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: `${color}25`, borderWidth: 2, borderColor: COLORS.backgroundCard, alignItems: 'center', justifyContent: 'center', marginLeft: i > 0 ? -6 : 0, zIndex: agentColors.length - i }}>
                    <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: color }} />
                  </View>
                ))}
              </View>
              <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs }}>{session.perspectives?.length ?? 0} agents debated</Text>
            </View>
          )}

          {/* FIX: Date overflow - use flexWrap and proper spacing */}
          <View style={{ 
            flexDirection: 'row', 
            alignItems: 'center', 
            flexWrap: 'wrap',
            ...
            {isCompleted && stanceAgainstCount > 0 && (
            ...
            <View style={{ backgroundColor: isCompleted ? `${COLORS.success}12` : isFailed ? `${COLORS.error}12` : `${COLORS.warning}12`, borderRadius: RADIUS.full, paddingHorizontal: 8, paddingVertical: 3 }}>
              <Text style={{ color: isCompleted ? COLORS.success : isFailed ? COLORS.error : COLORS.warning, fontSize: FONTS.sizes.xs, fontWeight: '700' }}>
            ...
            <View style={{ 
              ...
              <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs, flexShrink: 0 }}>{dateLabel}</Text>
            </View>
          </View>
        </View>
      </TouchableOpacity>
...
export default function DebateScreen() {
  ...
  const [collectionTarget, setCollectionTarget] = useState<DebateSession | null>(null);
  ...
  const handleCancel = useCallback(() => {
    Alert.alert('Cancel Debate', 'Stop generating this debate?',
    ...
    <LinearGradient colors={[COLORS.background, ...getAuroraGradient()] as [string, string, string]} style={{ flex: 1 }}>
          ...
          >
            ...
            <Animated.View entering={FadeIn.duration(600)} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: SPACING.xl }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.sm }}>
                <TouchableOpacity
                  ...
                  style={{ width: 36, height: 36, borderRadius: 11, backgroundColor: COLORS.backgroundElevated, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: COLORS.border }}
                >
            ...
            {showBanner && genState.session && (
              <Animated.View entering={FadeIn.duration(500)} style={{ marginBottom: SPACING.lg }}>
                <LinearGradient colors={[`${COLORS.primary}22`, `${COLORS.accent}18`] as [string, string]}
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: SPACING.sm }}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.sm, fontWeight: '700', marginTop: 2 }} numberOfLines={1}>{genState.session.topic}</Text>
                    </View>
            ...
            {showProgress && (
              <DebateProgressIndicator
                agentProgress={genState.agentProgress} progressPercent={progressPercent}
                progressMessage={genState.progressMessage} isModerating={phase === 'moderating'}
            ...
            {voiceState.error && (
              <Animated.View entering={FadeIn.duration(300)} style={{ backgroundColor: `${COLORS.warning}10`, borderRadius: RADIUS.lg, padding: SPACING.sm, marginBottom: SPACING.md, borderWidth: 1, borderColor: `${COLORS.warning}30`, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <TouchableOpacity onPress={clearVoiceError}><Ionicons name="close" size={16} color={COLORS.warning} /></TouchableOpacity>
              </Animated.View>
            )}
            ...
            {showForm && (
              <Animated.View entering={FadeInDown.duration(400).delay(100)}>
                ...
                <View style={{ backgroundColor: COLORS.backgroundCard, borderRadius: RADIUS.lg, padding: SPACING.md, marginBottom: SPACING.md, borderWidth: 1, borderColor: COLORS.border }}>
                  ...
                  <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs, fontWeight: '700', letterSpacing: 0.6, textTransform: 'uppercase', marginBottom: SPACING.sm }}>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACING.sm }}>
                    {[
                      ...
                      <View key={agent.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: `${agent.color}12`, borderRadius: RADIUS.full, paddingHorizontal: 10, paddingVertical: 5, borderWidth: 1, borderColor: `${agent.color}25` }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: SPACING.sm }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                    ...
                    <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs }}>
                      This will use{' '}
                      <Text style={{ color: COLORS.primary, fontWeight: '700' }}>{FEATURE_COSTS.debate} credits</Text>
                    </Text>
                  </View>
                  <CreditBalance balance={balance} size="sm" />
                </View>
      ...
      {collectionTarget && (
        <AddToCollectionSheet
          ...
          contentTitle={collectionTarget.topic}
