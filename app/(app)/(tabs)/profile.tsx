...
// Part 24 — UPDATED: Added "Credits & Billing" section showing live balance,
// quick stats, and a "Buy Credits" button that opens the credits store.
// FIX: Modals lifted outside SafeAreaView/LinearGradient to prevent clipping.
        ...
        {(isEmpty || isLow) && (
          <View style={{
            ...
            <Text style={{ color: accentColor, fontSize: FONTS.sizes.xs, fontWeight: '600' }}>
              {isEmpty
                ? 'No credits left — buy a pack to keep researching'
...
export default function ProfileScreen() {
  ...
  const handleCacheModalClose = useCallback(() => {
    ...
    // Reload stats after modal closes so row reflects any changes made inside
    loadCacheStats();
  }, [loadCacheStats]);
      ...
      if (value) {
        ...
        if (result === 'enabled') {
          ...
          Alert.alert(
            ...
            'Tap "Open Settings" to allow them.',
            [
  ...
  // ── Render ─────────────────────────────────────────────────────────────
  //
  // IMPORTANT: Both modals are rendered as siblings of the LinearGradient
  // root, NOT nested inside SafeAreaView. This prevents the SafeAreaView
  // from clipping the modal overlay on iOS and Android.
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* ─────────────────────────────────────────────────────────────────
          Modals are rendered OUTSIDE SafeAreaView so they are never
          clipped by the safe-area insets or the LinearGradient container.
          React Native's Modal already draws over the entire screen, but
          it must be a sibling (or ancestor-level) of SafeAreaView to
          prevent any overflow/clip styles from the parent cutting it off.
      ───────────────────────────────────────────────────────────────── */}
        ...
        <BlurView intensity={20} style={{ flex: 1, backgroundColor: 'rgba(10,10,26,0.7)', justifyContent: 'flex-end' }}>
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <View style={{
              ...
              padding: SPACING.xl, borderTopWidth: 1, borderTopColor: COLORS.border, maxHeight: '90%',
            }}>
