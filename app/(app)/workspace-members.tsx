...
// Part 46 UPDATE — useWorkspaceMembers now uses useWorkspaceRealtime
// internally, so role changes, removals, and blocks reflect instantly
// on this screen for ALL members viewing it simultaneously.
//
// Changes from Part 13B:
//   • block() is now called directly from useWorkspaceMembers hook
//     (no longer calls RPC directly from screen).
//   • Role badge updates in real time when owner changes roles.
//   • If the current user is removed/blocked while on this screen,
//     their role disappears and a toast-style banner is shown.
...
import { useBlockedMembers }            from '../../src/hooks/useBlockedMembers';
...
import { BlockedMembersModal }          from '../../src/components/workspace/BlockedMembersModal';
...
export default function WorkspaceMembersScreen() {
  ...
  const handleBlockMember = (member: WorkspaceMember) => {
    ...
    Alert.alert(
      'Block Member',
      `Block ${name}?\n\nThey will be removed immediately and cannot rejoin this workspace. You can unblock them later.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
  ...
  const handleLeave = () => {
    Alert.alert(
      'Leave Workspace',
      'Are you sure you want to leave? You will lose access to all shared reports and comments.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
  ...
  const handleApproveRequest = async (requestId: string) => {
    ...
    if (error) Alert.alert('Error', error);
    else { Alert.alert('✅ Approved', 'Member has been upgraded to Editor.'); refresh(); }
  };
              ...
              {sections.map(section => (
                <Animated.View key={section.role} entering={FadeInDown.duration(400)}>
                  ...
                  {section.data.map((member, i) => {
                    ...
                    const joinedDate = new Date(member.joinedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                          ...
                          <View style={styles.memberRight}>
                            ...
                            {canManage && (
                              <View style={styles.memberActions}>
                                <TouchableOpacity onPress={() => handleTransfer(member)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} style={[styles.iconBtn, { backgroundColor: `${COLORS.warning}15` }]}>
                                  <Ionicons name="key-outline" size={14} color={COLORS.warning} />
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => handleManageMember(member)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} style={[styles.iconBtn, { backgroundColor: `${COLORS.primary}15` }]}>
                                  <Ionicons name="settings-outline" size={14} color={COLORS.primary} />
                                </TouchableOpacity>
                                <TouchableOpacity onPress={() => handleBlockMember(member)} disabled={isBlocking} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} style={[styles.iconBtn, { backgroundColor: `${COLORS.error}12` }]}>
