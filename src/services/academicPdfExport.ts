...
// Part 41.8 — renderSection now handles any section type string, not just the
//              7 canonical AcademicSectionType values. Custom sections
//              (discussion, limitations, future_research, etc.) render as
//              standard sections with their title and content.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Section renderer ─────────────────────────────────────────────────────────
// Part 41.8: uses section.type === 'abstract' / 'references' guards;
// ALL other types (including custom) fall through to the standard section block.
...
function renderSection(section: AcademicSection): string {
  ...
  if (section.type === 'references') {
    ...
    const listItems = lines.map(l => `<p class="reference-item">${escapeHtml(l)}</p>`).join('\n');
    parts.push(`
      <div class="section">
        <h2 class="section-heading">${escapeHtml(section.title)}</h2>
  ...
  // Standard section — handles all canonical + custom section types
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      ...
      font-size: 12pt; line-height: 2; color: #000; background: #fff; margin: 0; padding: 0;
    }
    .page { max-width: 680px; margin: 0 auto; padding: 72px; }
    .running-head {
      font-family: Arial, Helvetica, sans-serif; font-size: 9pt;
      letter-spacing: 0.04em; text-transform: uppercase; color: #555;
      border-bottom: 1px solid #ddd; padding-bottom: 8px; margin-bottom: 32px;
    ...
    .paper-title {
      ...
      font-weight: 700; line-height: 1.3; color: #1a1a2e; margin-bottom: 20px;
    }
    .paper-meta { font-family: Arial, Helvetica, sans-serif; font-size: 9pt; color: #555; margin-bottom: 8px; }
    .keywords-block {
      font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #333;
      margin-top: 16px; text-align: left; background: #f8f8ff;
      border-left: 3px solid #6c63ff; padding: 10px 16px; border-radius: 0 4px 4px 0;
    }
    .keywords-label { font-weight: 700; font-style: italic; }
    .abstract-block {
      background: #f8f8ff; border: 1px solid #e0e0f0; border-radius: 6px;
      padding: 24px 28px; margin-bottom: 40px;
    }
    .abstract-block .section-heading {
      text-align: center; font-family: Arial, Helvetica, sans-serif;
      font-size: 13pt; font-weight: 700; margin-bottom: 16px; color: #1a1a2e; border-bottom: none;
    }
    .abstract-block p { font-size: 11pt; line-height: 1.8; color: #222; text-align: justify; text-indent: 0; }
    .section { margin-bottom: 32px; }
    .section-heading {
      font-family: Arial, Helvetica, sans-serif; font-size: 13pt; font-weight: 700;
      color: #1a1a2e; border-bottom: 2px solid #6c63ff;
      padding-bottom: 6px; margin-bottom: 16px; margin-top: 32px;
    }
    .subsection-heading {
      font-family: Arial, Helvetica, sans-serif; font-size: 11pt;
      font-weight: 700; font-style: italic; color: #2d2d50;
    ...
    .reference-item {
      text-indent: -2em !important; padding-left: 2em; margin-bottom: 10px;
      font-size: 11pt; line-height: 1.6; color: #222;
    }
    .stats-bar { display: flex; gap: 24px; justify-content: center; margin-top: 16px; padding: 10px 0; }
    .stat-item { text-align: center; font-family: Arial, Helvetica, sans-serif; }
    .stat-value { font-size: 13pt; font-weight: 700; color: #6c63ff; display: block; }
    .stat-label { font-size: 8pt; color: #888; text-transform: uppercase; letter-spacing: 0.05em; }
    .footer {
      margin-top: 48px; padding-top: 12px; border-top: 1px solid #ddd;
      font-family: Arial, Helvetica, sans-serif; font-size: 8pt; color: #aaa; text-align: center;
    }
  </style>
</head>
<body>
