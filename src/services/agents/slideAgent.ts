    ...
    additionalBlocks: [
      {
        ...
        chart:    { id: '__placeholder__', type: 'bar', title: 'Chart Placeholder', datasets: [], labels: [] },
...
export async function runSlideAgent(
  ...
  const findingsContext = report.keyFindings.slice(0, 6).map((f, i) => `${i + 1}. ${f}`).join('\n');
  const statsContext = report.statistics.slice(0, 8).map(s => `${s.value}: ${s.context} (${s.source})`).join('\n');
  const predictionsContext = report.futurePredictions.slice(0, 5).map((p, i) => `${i + 1}. ${p}`).join('\n');
  const citationsContext = report.citations.slice(0, 5)
    .map((c, i) => `[${i + 1}] ${c.title} — ${c.source}${c.date ? ` (${c.date})` : ''} — ${c.url}`).join('\n');
...
STRICT JSON FORMAT:
{
  ...
  const slides: Omit<PresentationSlide, 'slideNumber'>[] = raw.slides.map((s, i) => {
    const layout: SlideLayout = (
      ['title','agenda','section','content','bullets','stats','quote','chart_ref','predictions','references','closing']
        .includes(s.layout) ? s.layout : 'content'
    ) as SlideLayout;

    const editorData = layout === 'chart_ref' ? buildChartRefEditorData() : undefined;

    return {
      id:               s.id ?? `slide_${String(i + 1).padStart(3, '0')}`,
      layout,
      title:            (typeof s.title === 'string' && s.title.trim()) ? s.title.trim() : `Slide ${i + 1}`,
      ...
      icon:             typeof s.icon === 'string' ? s.icon : LAYOUT_ICON[layout],
