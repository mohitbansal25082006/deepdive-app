...
// Part 17 — Workspace Chat hook (REPLY FIX)
//
// Root cause of reply not working:
//   send() was a useCallback with [state.messages, state.isSending, ...]
//   in its dependency array.  Every setState (setting replyingTo, isSending,
//   etc.) recreated send() with a new closure.  Because handleSend in
//   workspace-chat.tsx captured `chat` via useCallback([chat, sendTyping]),
//   it sometimes held a stale send() that no longer had the right
//   state.messages reference — so replyToId arrived correctly but
//   replyMsg lookup returned undefined, and the optimistic replyTo was null.
//   React batched the replyingTo → null setState before the new send()
//   ran, so the reply preview was stripped.
//
// Fix:
//   Use a stateRef (useRef) that is always kept in sync with the latest
//   state.  All callbacks read from stateRef.current instead of closed-over
//   state, so they NEVER go stale regardless of how often state updates.
//   This is the standard pattern for stable async callbacks in React hooks.
...
export function useWorkspaceChat(workspaceId: string | null) {
  ...
  // ── stateRef: always mirrors latest state so callbacks never go stale ───────
  const stateRef = useRef<ChatState>(INITIAL_STATE);
  useEffect(() => { stateRef.current = state; }, [state]);

  // ── authRef: same idea for user/profile ─────────────────────────────────────
  ...
  useEffect(() => {
    ...
    // Reset to initial on workspace change
    ...
    const unsub = subscribeToChatMessages(workspaceId, {
      onInsert: (msg) => {
        ...
        setState(s => {
          // Already confirmed
          if (s.messages.some(m => m.id === msg.id && !m.id.startsWith('temp-'))) {
            return s;
          }

          // Replace matching optimistic temp message
          const tempIdx = s.messages.findIndex(
            m =>
        ...
        if (msg.userId !== u?.id && workspaceIdRef.current) {
          markMessagesRead(workspaceIdRef.current, msg.id);
        }
      },
      ...
      onUpdate: (partial) => {
        setState(s => ({
          ...s,
          messages: s.messages.map(m => m.id === partial.id ? { ...m, ...partial } : m),
        }));
      },
  ...
  // ── Send message ─────────────────────────────────────────────────────────────
  //
  // KEY FIX: replyToId comes in as a parameter (passed from ChatInput via
  // handleSend in the screen).  We NO LONGER read state.replyingTo inside
  // this function — the caller is responsible for passing the id.
  // This means the function is fully stable (no state deps) and the reply
  // id is never lost to a stale closure.

  const send = useCallback(async (
    content:      string,
    replyToId?:   string,   // << explicit param, never read from state
    ...
    // Allow attachment-only messages: content can be empty if attachments provided
    const hasContent = content.trim().length > 0 || (attachments && attachments.length > 0);
    if (!wsId || !hasContent || s.isSending) return;

    // Capture replyToId and the reply message BEFORE any setState
    const capturedReplyId = replyToId ?? null;
    const replyMsg = capturedReplyId
      ? s.messages.find(m => m.id === capturedReplyId)
      : undefined;
    ...
    // Set isSending + clear replyingTo + add optimistic message — all at once
    setState(prev => ({
      ...prev,
      isSending:  true,
      replyingTo: null,
    ...
    // Call server with the captured reply id
  ...
  }, []);  // fully stable — no state deps, uses refs only
  ...
  const pin = useCallback(async (message: ChatMessage) => {
    ...
    if (!error) {
      ...
      const wsId = workspaceIdRef.current;
      if (wsId) {
        const { data } = await getPinnedChatMessages(wsId);
  ...
  const search = useCallback(async (query: string) => {
    ...
    if (!query.trim()) {
      setState(s => ({ ...s, searchResults: [], isSearching: false }));
      return;
    }
  ...
  const setReplyingTo = useCallback((msg: ChatMessage | null) => {
    setState(s => ({ ...s, replyingTo: msg }));
  }, []);

  const setEditingMessage = useCallback((msg: ChatMessage | null) => {
    setState(s => ({ ...s, editingMessage: msg }));
  }, []);

  // ── Refresh ───────────────────────────────────────────────────────────────────

  const refresh = useCallback(async () => {
    await Promise.all([loadMessages(true), loadAuxData()]);
