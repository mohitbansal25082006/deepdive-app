...
// Results grouped by type with tap-to-navigate support.
...
import { WorkspaceSearchResult, WorkspaceRole } from '../../types';
import { COLORS, FONTS, SPACING, RADIUS } from '../../constants/theme';

// ─── Icon + colour map per result type ───────────────────────────────────────
...
interface Props {
  ...
  onClose:      () => void;
  /** Called when user taps a report result */
...
export function WorkspaceSearchModal({
  ...
  const { query, results, isSearching, error, search, clear } =
  ...
  const handleResultPress = (item: WorkspaceSearchResult) => {
    ...
    if (item.type === 'report' && item.reportId) {
      ...
      handleClose();
    } else if (item.type === 'comment' && item.reportId) {
      onOpenReport(item.reportId);
      handleClose();
    }
    // member taps — no dedicated screen, just dismiss
  };
  ...
  const grouped: Array<{ sectionType: WorkspaceSearchResult['type']; data: WorkspaceSearchResult[] }> = [
  ...
  return (
    <View style={stateStyles.wrap}>
      ...
      <View style={stateStyles.tips}>
        {['Try "AI trends"', 'Search by member name', 'Find a comment keyword'].map((tip) => (
