...
//   • The 8-entry hardcoded SESSION_GRADIENTS array → kbGradientForIndex() from
//     the live palette.
//   • Hardcoded brand gradients ['#6C63FF','#8B5CF6'] / ['#7C3AED','#6C63FF'] →
//     kbBrandGradient() (COLORS.gradientPrimary).
//   • styles → makeStyles() factory; SessionCard + EmptyState take the live
//     styles object; the panel subscribes to useTheme().
...
import { kbGradientForIndex, kbBrandGradient } from './kbTheme';
...
function SessionCard({ session, index, isActive, onSelect, onRename, onDelete, styles }: SessionCardProps) {
  ...
  return (
    <Animated.View entering={FadeIn.duration(250)}>
      ...
      >
        {isActive && (
          <LinearGradient colors={gradient as [string, string]} style={styles.cardAccentBar} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} />
        )}

        {/* Icon orb — matches depth card icon orb style */}
        <LinearGradient colors={gradient as [string, string]} style={styles.cardAvatar}>
...
// ─── Empty state ──────────────────────────────────────────────────────────────
function EmptyState({ onNew, styles }: { onNew: () => void; styles: Styles }) {
  ...
  return (
    <View style={styles.emptyWrap}>
      ...
      <Pressable onPress={onNew} style={styles.emptyBtn}>
        <LinearGradient colors={brand as [string, string]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.emptyBtnGrad}>
...
export function KBSessionsPanel({ visible, activeSessionId, onClose, onSelectSession, onNewSession }: Props) {
  // Part 55.1: subscribe so the panel recolours on a theme switch.
  useTheme();
          ...
          <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            <View style={styles.panelHeader}>
              ...
              <View style={styles.panelTitleRow}>
                <LinearGradient colors={brand as [string, string]} style={styles.panelIcon}>
            ...
            <Pressable onPress={handleNew} style={({ pressed }) => [styles.newChatBtn, pressed && { opacity: 0.85 }]}>
              <LinearGradient colors={brand as [string, string]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.newChatGrad}>
            ...
            {isLoading && sessions.length === 0 ? (
              ...
              searchQuery
                ...
                : <EmptyState onNew={handleNew} styles={styles} />
            ) : (
              <FlatList
