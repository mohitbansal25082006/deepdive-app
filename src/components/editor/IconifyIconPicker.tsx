...
// Part 30 — FIXED
// Bug fix: Icons showed blank because React Native's Image component cannot
// render SVG URLs. Fix: fetch SVG XML from Iconify API and render with SvgXml
// from react-native-svg. This correctly colours and sizes every icon.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Recolour SVG XML ─────────────────────────────────────────────────────────
// Iconify SVGs use currentColor — we just set color on SvgXml.
// For SVGs that have hardcoded fill/stroke, we do a simple string replace.
...
// ─── Icon Cell — fetches and renders SVG via SvgXml ──────────────────────────
...
export function IconifyIconPicker({
  ...
  // When an icon is selected, fetch its SVG for the detail preview
  ...
  // When color changes, recolour the preview SVG
  useEffect(() => {
    if (selectedSvgXml && selected) {
  ...
  const handleInsert = useCallback(() => {
    ...
    const block: IconBlock = {
      type:      'icon',
      id:        uid(),
      iconName:  'shapes-outline', // Ionicons fallback
  ...
  // Build rows of COLS for the icon grid (avoid FlatList-in-ScrollView)
  ...
  return (
    ...
    >
      <Pressable
        ...
        onPress={!showDetail ? onClose : undefined}
      >
        <Pressable
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, marginBottom: SPACING.md }}>
            {showDetail && (
              <Pressable
                ...
                style={{ width: 34, height: 34, borderRadius: 10, backgroundColor: COLORS.backgroundElevated, alignItems: 'center', justifyContent: 'center', marginRight: SPACING.sm, borderWidth: 1, borderColor: COLORS.border }}
              >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs }}>
                {showDetail && selected ? `${selected.prefix} · customise & place` : '275,000+ icons · tap to browse'}
              </Text>
            </View>
          ...
          {showDetail && selected ? (
            ...
            >
              {/* Big preview */}
              <Animated.View entering={FadeIn.duration(250)} style={{ alignItems: 'center', paddingVertical: SPACING.lg }}>
                <View style={{ width: 96, height: 96, borderRadius: 24, backgroundColor: `${color}18`, borderWidth: 2, borderColor: `${color}35`, alignItems: 'center', justifyContent: 'center' }}>
                  {selectedSvgXml ? (
                ...
                <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.base, fontWeight: '700', marginTop: 10 }}>{selected.name}</Text>
                <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs, marginTop: 2 }}>{selected.prefix} · {selected.id}</Text>
              </Animated.View>
              ...
              <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: COLORS.border, paddingHorizontal: SPACING.md }}>
                <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs, marginTop: 8 }}>Label (optional)</Text>
                <TextInput
                  value={label}
              ...
              {/* Size */}
              <View style={{ gap: 8 }}>
                <Text style={{ color: COLORS.textMuted, fontSize: 9, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase' }}>Icon Size</Text>
                <View style={{ flexDirection: 'row', gap: SPACING.sm }}>
                  {[24, 32, 40, 56, 72].map(sz => (
                    <Pressable
                      key={sz}
                      onPress={() => setIconSize(sz)}
                      style={{ flex: 1, alignItems: 'center', paddingVertical: 8, borderRadius: RADIUS.lg, backgroundColor: iconSize === sz ? `${COLORS.primary}18` : COLORS.backgroundElevated, borderWidth: 1.5, borderColor: iconSize === sz ? COLORS.primary : COLORS.border }}
                    >
                      <Text style={{ color: iconSize === sz ? COLORS.primary : COLORS.textSecondary, fontSize: FONTS.sizes.xs, fontWeight: '700' }}>{sz}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
              ...
              {/* Color */}
              <View style={{ gap: 8 }}>
                <Text style={{ color: COLORS.textMuted, fontSize: 9, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase' }}>Icon Color</Text>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                  {ACCENT_COLORS.map(c => (
                    <Pressable
                      key={c}
                      onPress={() => setColor(c)}
                      style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: c, borderWidth: color === c ? 3 : 1, borderColor: color === c ? '#FFF' : 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' }}
                    >
                      {color === c && <Ionicons name="checkmark" size={15} color={c === '#FFFFFF' ? '#000' : '#FFF'} />}
                    </Pressable>
                  ))}
                </View>
              </View>
              ...
              {/* Placement */}
              <View style={{ gap: 8 }}>
                <Text style={{ color: COLORS.textMuted, fontSize: 9, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase' }}>Placement on Slide</Text>
              ...
              <TouchableOpacity onPress={handleInsert} activeOpacity={0.8}>
                <LinearGradient
                  ...
                  style={{ borderRadius: RADIUS.full, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10 }}
                >
                  {selectedSvgXml
                    ? <SvgXml xml={selectedSvgXml} width={20} height={20} color="#FFF" />
                    : <Ionicons name="shapes-outline" size={20} color="#FFF" />
                  }
                  <Text style={{ color: '#FFF', fontSize: FONTS.sizes.base, fontWeight: '800' }}>Add Icon to Slide</Text>
                </LinearGradient>
              </TouchableOpacity>
            </ScrollView>
            ...
            <>
              {/* Search bar */}
              <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, marginHorizontal: SPACING.lg, marginBottom: SPACING.sm, paddingHorizontal: SPACING.md, borderWidth: 1, borderColor: query ? COLORS.primary : COLORS.border }}>
                ...
                />
                {query.length > 0 && (
                  <Pressable onPress={() => { setQuery(''); loadCategory(activeCategory); }}>
              ...
              {!query && (
                <ScrollView
                  ...
                  contentContainerStyle={{ paddingHorizontal: SPACING.lg, gap: SPACING.sm, alignItems: 'center', marginBottom: SPACING.sm }}
                  ...
                  {ICONIFY_CATEGORIES.map(cat => {
                    ...
                    return (
                      <Pressable
                        ...
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: isActive ? `${COLORS.primary}18` : COLORS.backgroundElevated, borderRadius: RADIUS.full, paddingHorizontal: 12, paddingVertical: 7, borderWidth: 1, borderColor: isActive ? COLORS.primary : COLORS.border, flexShrink: 0 }}
                      >
                        <Text style={{ color: isActive ? COLORS.primary : COLORS.textSecondary, fontSize: FONTS.sizes.xs, fontWeight: isActive ? '700' : '500' }}>{cat.label}</Text>
                      </Pressable>
                    );
                  })}
                </ScrollView>
              ...
              {/* Icon grid — plain View rows to avoid FlatList-in-ScrollView */}
              {isLoading ? (
                <View style={{ alignItems: 'center', paddingVertical: SPACING.xl }}>
                  <ActivityIndicator size="large" color={COLORS.primary} />
                  <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.sm, marginTop: SPACING.sm }}>Loading icons…</Text>
                </View>
              ) : (
                <ScrollView
                  showsVerticalScrollIndicator={false}
                  style={{ maxHeight: SCREEN_H * 0.42 }}
                  contentContainerStyle={{ paddingHorizontal: SPACING.lg, paddingBottom: SPACING.sm, gap: SPACING.xs }}
                >
                  {iconRows.map((row, rowIdx) => (
                    <View key={rowIdx} style={{ flexDirection: 'row', gap: SPACING.xs, marginBottom: SPACING.xs }}>
                      {row.map(icon => (
                        <IconCell
                          key={icon.id}
                          icon={icon}
