...
// Changes:
//   1. Category tab strip: fixed squishing — each chip now uses flexShrink:0
//      so tabs are never compressed on narrow screens
//   2. All other logic identical to Part 29
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    ...
    >
      <View style={{ borderRadius: 0, overflow: 'hidden', position: 'relative' }}>
        ...
        {template.tag && (
          <View style={{ position: 'absolute', top: 6, right: 6, backgroundColor: accentColor, borderRadius: RADIUS.full, paddingHorizontal: 7, paddingVertical: 2 }}>
            <Text style={{ color: '#FFF', fontSize: 8, fontWeight: '800' }}>{template.tag}</Text>
          </View>
        )}
        <View style={{ position: 'absolute', bottom: 6, left: 6, flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: 'rgba(0,0,0,0.6)', borderRadius: RADIUS.full, paddingHorizontal: 7, paddingVertical: 2 }}>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
          <View style={{ width: 20, height: 20, borderRadius: 6, backgroundColor: template.gradient[0] + '30', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, paddingVertical: SPACING.sm, borderBottomWidth: 1, borderBottomColor: COLORS.border, gap: SPACING.sm }}>
        <Pressable onPress={onBack} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }} style={{ width: 34, height: 34, borderRadius: 10, backgroundColor: COLORS.backgroundElevated, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: COLORS.border }}>
          <Ionicons name="arrow-back" size={18} color={COLORS.textSecondary} />
        </Pressable>
        <LinearGradient colors={template.gradient} style={{ width: 34, height: 34, borderRadius: 10, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs }}>
            {template.slideCount} slides{template.suggestedTheme ? ` · Best with ${THEME_LABELS[template.suggestedTheme]}` : ''}
          </Text>
        </View>
        {template.tag && (
          <LinearGradient colors={template.gradient} style={{ borderRadius: RADIUS.full, paddingHorizontal: 10, paddingVertical: 4 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: insets.bottom + 120 }}>
        ...
        <Animated.View entering={FadeIn.duration(250)} style={{ marginHorizontal: SPACING.lg, marginTop: SPACING.lg }}>
          <View style={{ borderRadius: 14, overflow: 'hidden', borderWidth: 2, borderColor: `${accentColor}50`, shadowColor: accentColor, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.3, shadowRadius: 18, elevation: 10 }}>
            <SlideCard slide={currentSlide} tokens={tokens} scale={DETAIL_SCALE} />
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: SPACING.sm }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: `${accentColor}18`, borderRadius: RADIUS.full, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: `${accentColor}35` }}>
            ...
            <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs }}>{previewIdx + 1} / {template.slideCount}</Text>
          </View>
        </Animated.View>

        {/* Thumbnail strip
            Part 30 FIX: same approach — flexShrink:0 on each thumb prevents squishing */}
        <ScrollView
          horizontal
          showsHorizontalScrollIndicator={false}
          contentContainerStyle={{ paddingHorizontal: SPACING.lg, paddingVertical: SPACING.md, gap: SPACING.sm, alignItems: 'flex-start' }}
          style={{ marginTop: SPACING.sm }}
        >
          {previewSlides.map((slide, i) => (
            <Pressable key={i} onPress={() => setPreviewIdx(i)} style={{ flexShrink: 0 }}>
              <View style={{ borderRadius: 6, overflow: 'hidden', borderWidth: 2, borderColor: i === previewIdx ? accentColor : 'transparent', shadowColor: i === previewIdx ? accentColor : '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: i === previewIdx ? 0.4 : 0.1, shadowRadius: 6, elevation: i === previewIdx ? 6 : 2 }}>
                <SlideCard slide={slide} tokens={tokens} scale={THUMB_SCALE} />
              </View>
              <Text style={{ color: i === previewIdx ? accentColor : COLORS.textMuted, fontSize: 9, textAlign: 'center', marginTop: 3, fontWeight: i === previewIdx ? '700' : '500' }}>{i + 1}</Text>
            </Pressable>
          ))}
        </ScrollView>
        ...
        <View style={{ marginHorizontal: SPACING.lg, gap: SPACING.md }}>
          <LinearGradient
            ...
            style={{ borderRadius: RADIUS.xl, padding: SPACING.md, borderWidth: 1, borderColor: template.gradient[0] + '25', gap: SPACING.sm }}
          >
            ...
            <View style={{ gap: 4 }}>
              {template.slides.map((ts, i) => (
                <Pressable key={i} onPress={() => setPreviewIdx(i)} style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.sm, paddingVertical: 6, paddingHorizontal: SPACING.sm, backgroundColor: i === previewIdx ? `${accentColor}15` : 'transparent', borderRadius: RADIUS.md }}>
                  <View style={{ width: 22, height: 22, borderRadius: 6, backgroundColor: i === previewIdx ? accentColor : COLORS.backgroundElevated, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Text style={{ color: i === previewIdx ? '#FFF' : COLORS.textMuted, fontSize: 9, fontWeight: '800' }}>{i + 1}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text numberOfLines={1} style={{ color: i === previewIdx ? COLORS.textPrimary : COLORS.textSecondary, fontSize: FONTS.sizes.xs, fontWeight: '600' }}>{ts.title}</Text>
                    <Text style={{ color: COLORS.textMuted, fontSize: 9, textTransform: 'uppercase', letterSpacing: 0.5 }}>{ts.layout.replace('_', ' ')}</Text>
                  </View>
          ...
          {currentSlide.speakerNotes && (
            <View style={{ backgroundColor: `${COLORS.info}10`, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: `${COLORS.info}25` }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                ...
                <Text style={{ color: COLORS.info, fontSize: FONTS.sizes.xs, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 }}>Speaker Notes — Slide {previewIdx + 1}</Text>
              </View>
      ...
      {/* Action bar */}
      <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: SPACING.lg, paddingTop: SPACING.md, paddingBottom: insets.bottom + SPACING.md, backgroundColor: COLORS.backgroundCard, borderTopWidth: 1, borderTopColor: COLORS.border, gap: SPACING.sm }}>
        <TouchableOpacity onPress={onInsert} activeOpacity={0.8} style={{ borderRadius: RADIUS.full, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: `${accentColor}18`, borderWidth: 1.5, borderColor: `${accentColor}50` }}>
          <Ionicons name="add-circle-outline" size={19} color={accentColor} />
          <View style={{ alignItems: 'center' }}>
            <Text style={{ color: accentColor, fontSize: FONTS.sizes.base, fontWeight: '800' }}>Insert {template.slideCount} Slides After Current</Text>
            <Text style={{ color: `${accentColor}AA`, fontSize: 10, marginTop: 1 }}>After slide {activeSlideIndex + 1} · Non-destructive</Text>
          </View>
        </TouchableOpacity>
        <TouchableOpacity onPress={onApply} activeOpacity={0.8}>
          <LinearGradient colors={template.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ borderRadius: RADIUS.full, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, ...SHADOWS.medium }}>
            <Ionicons name="copy-outline" size={19} color="#FFF" />
            <View style={{ alignItems: 'center' }}>
              <Text style={{ color: '#FFF', fontSize: FONTS.sizes.base, fontWeight: '800' }}>Apply Entire Template</Text>
              <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 10, marginTop: 1 }}>Replaces all slides · Snapshot saved in History</Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
      </View>
...
export function TemplateLibrary({
  ...
  const handleOpenDetail = useCallback((template: SlideTemplate) => setDetailTemplate(template), []);
  ...
  const handleBack = useCallback(() => setDetailTemplate(null), []);

  const renderTemplate = useCallback(({ item }: { item: SlideTemplate }) => (
    <TemplateGridCard template={item} theme={currentTheme} onPress={handleOpenDetail} />
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'flex-end' }} onPress={!detailTemplate ? onClose : undefined}>
        <Pressable
          ...
          {detailTemplate ? (
            ...
            <>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.lg, marginBottom: SPACING.md }}>
                <LinearGradient colors={['#43E97B', '#38F9D7']} style={{ width: 34, height: 34, borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginRight: SPACING.sm }}>
                  <Ionicons name="copy-outline" size={17} color="#FFF" />
                </LinearGradient>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.base, fontWeight: '800' }}>Template Library</Text>
                  <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs }}>{TEMPLATE_COUNT} professional templates · History auto-saved</Text>
                </View>
              ...
              {/* Category tabs
                  Part 30 FIX: flexShrink:0 on each chip prevents squishing */}
              <ScrollView
                horizontal
                showsHorizontalScrollIndicator={false}
                  ...
                  return (
                    <Pressable
                      ...
                      style={{
                        ...
                        // Part 30 FIX: prevent horizontal squishing
                        flexShrink:        0,
                      }}
                    >
                      ...
                      <View style={{ backgroundColor: active ? COLORS.primary : COLORS.backgroundCard, borderRadius: RADIUS.full, paddingHorizontal: 5, paddingVertical: 1, minWidth: 18, alignItems: 'center' }}>
                        <Text style={{ color: active ? '#FFF' : COLORS.textMuted, fontSize: 8, fontWeight: '800' }}>{count}</Text>
                      </View>
                    </Pressable>
                  );
                })}
