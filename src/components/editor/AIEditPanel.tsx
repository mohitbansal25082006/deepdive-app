...
// Part 28 — AI editing panel
// FIX: Bullet rewrite section added to Rewrite tab.
//      Shows "Rewrite All Bullets" when slide has bullets.
//      Shows per-bullet rewrite buttons for individual items.
// ─────────────────────────────────────────────────────────────────────────────

import React, { useState, useCallback } from 'react';
import {
  View, Text, Modal, Pressable, ScrollView,
  TextInput, ActivityIndicator, Dimensions, TouchableOpacity,
} from 'react-native';
  ...
  return (
    <View style={{
      ...
      >
        ...
        <Text style={{ flex: 1, color: COLORS.textSecondary, fontSize: FONTS.sizes.sm, lineHeight: 18 }} numberOfLines={2}>
          {text || `Bullet ${bulletIndex + 1} (empty)`}
        </Text>
        <Ionicons
      ...
      {open && (
        <View style={{
          ...
          {AI_REWRITE_OPTIONS.map(opt => (
            <TouchableOpacity
              key={opt.id}
              onPress={() => { if (canAfford) { onRewrite(bulletIndex, opt.id); setOpen(false); } }}
              disabled={!canAfford || !text.trim()}
              activeOpacity={0.7}
              style={{
                flexDirection:   'row',
                alignItems:      'center',
                gap:             5,
                backgroundColor: `${opt.gradient[0]}18`,
                borderRadius:    RADIUS.full,
                paddingHorizontal: 10,
                paddingVertical:   6,
                borderWidth:     1,
                borderColor:     `${opt.gradient[0]}35`,
                opacity:         canAfford && text.trim() ? 1 : 0.4,
              }}
            >
...
export function AIEditPanel({
  ...
  const handleGenerate = useCallback(() => {
    ...
    onGenerateSlide({
      ...
      insertAfterIdx: insertAfter === 'after' ? currentSlideIndex : currentSlideIndex - 1,
    });
    setGenPrompt('');
        ...
        >
          ...
          {isProcessing && (
            <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(10,10,26,0.93)', zIndex: 99, alignItems: 'center', justifyContent: 'center', gap: SPACING.lg, borderTopLeftRadius: 24, borderTopRightRadius: 24 }}>
              ...
              <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.base, fontWeight: '700', textAlign: 'center' }}>
                {processingLabel || 'AI is working…'}
              </Text>
          ...
          {layoutSuggestion && (
            <Animated.View entering={FadeInDown.duration(300)} style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.sm, marginHorizontal: SPACING.lg, marginBottom: SPACING.md, backgroundColor: `${COLORS.warning}12`, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: `${COLORS.warning}35` }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.sm, fontWeight: '700', marginBottom: 2 }}>
                  Try: {LAYOUT_LABELS[layoutSuggestion.suggestedLayout] ?? layoutSuggestion.suggestedLayout}
                </Text>
            ...
            {AI_TABS.map(tab => {
              ...
              return (
                <Pressable
                  ...
                  style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: active ? `${tab.gradient[0]}18` : COLORS.backgroundElevated, borderRadius: RADIUS.full, paddingHorizontal: 12, paddingVertical: 8, borderWidth: 1, borderColor: active ? tab.gradient[0] : COLORS.border }}
                >
          ...
          >
            ...
            {activeTab === 'rewrite' && (
              <>
                {/* ── Mode toggle: Text Fields vs Bullet Points ── */}
                {isBulletLayout && hasBullets && (
                  <View style={{ flexDirection: 'row', backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.xl, padding: 3, borderWidth: 1, borderColor: COLORS.border }}>
                    {(['all', 'individual'] as const).map(mode => (
                      <Pressable
                        key={mode}
                        onPress={() => setBulletMode(mode)}
                        style={{ flex: 1, alignItems: 'center', paddingVertical: 8, borderRadius: RADIUS.lg, backgroundColor: bulletMode === mode ? COLORS.primary : 'transparent' }}
                      >
                        <Text style={{ color: bulletMode === mode ? '#FFF' : COLORS.textMuted, fontSize: FONTS.sizes.xs, fontWeight: '700' }}>
                          {mode === 'all' ? '✦ All Bullets' : '⊞ Per Bullet'}
                        </Text>
                      </Pressable>
                    ))}
                  </View>
                ...
                {/* ── BULLET: Rewrite All ── */}
                {isBulletLayout && hasBullets && bulletMode === 'all' && (
                  <>
                    {/* Preview of current bullets */}
                    <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: `${COLORS.primary}30`, borderLeftWidth: 3, borderLeftColor: COLORS.primary, gap: 6 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                        ...
                        <Text style={{ color: COLORS.primary, fontSize: FONTS.sizes.xs, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 }}>
                          {currentSlide!.bullets!.length} bullets — rewrite all at once
                        </Text>
                      </View>
                      ...
                      {currentSlide!.bullets!.length > 4 && (
                        <Text style={{ color: COLORS.textMuted, fontSize: 10 }}>+{currentSlide!.bullets!.length - 4} more…</Text>
                      )}
                    </View>

                    {/* Rewrite style buttons for all bullets */}
                    <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 1 }}>
                      Choose rewrite style — applies to ALL bullets
                    </Text>
                    <View style={{ gap: SPACING.sm }}>
                      {AI_REWRITE_OPTIONS.map(opt => {
                        ...
                        return (
                          <TouchableOpacity
                            key={opt.id}
                            onPress={() => !disabled && onRewriteBullets(opt.id)}
                            disabled={disabled}
                            activeOpacity={0.75}
                            style={{ opacity: disabled ? 0.45 : 1 }}
                          >
                            <LinearGradient
                              colors={opt.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                              style={{ flexDirection: 'row', alignItems: 'center', borderRadius: RADIUS.xl, padding: SPACING.md, gap: SPACING.md }}
                            >
                ...
                {/* ── BULLET: Per-bullet rewrite ── */}
                {isBulletLayout && hasBullets && bulletMode === 'individual' && (
                  <>
                    <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 1 }}>
                      Tap any bullet to expand its rewrite options
                    </Text>
                    <View style={{ gap: SPACING.sm }}>
                      {currentSlide!.bullets!.map((bullet, i) => (
                        <BulletRewriteRow
                          ...
                          onRewrite={onRewriteSingleBullet}
                        />
                      ))}
                    </View>
                    {!canAffordRewrite && (
                      <View style={{ backgroundColor: `${COLORS.error}12`, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: `${COLORS.error}30`, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        ...
                        <Text style={{ color: COLORS.error, fontSize: FONTS.sizes.xs, flex: 1 }}>
                          You need at least 1 credit to rewrite. Go to Profile → Credits to top up.
                        </Text>
                      </View>
                    )}
                  </>
                ...
                {/* ── TEXT FIELDS (always shown; if bullet layout, shown below bullets) ── */}
                {(!isBulletLayout || !hasBullets) && (
                  <>
                    {/* Field picker */}
                    <View>
                      ...
                      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: SPACING.sm }}>
                        {availableFields.map(field => {
                          ...
                          return (
                            <Pressable
                              key={field}
                              onPress={() => setPickedField(field)}
                              style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: RADIUS.full, backgroundColor: isPicked ? `${COLORS.primary}18` : COLORS.backgroundElevated, borderWidth: 1.5, borderColor: isPicked ? COLORS.primary : COLORS.border, gap: 3, alignItems: 'center' }}
                            >
                              <Text style={{ color: isPicked ? COLORS.primary : COLORS.textSecondary, fontSize: FONTS.sizes.xs, fontWeight: isPicked ? '700' : '500' }}>
                    ...
                    {effectiveField && effectiveValue ? (
                      <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: `${COLORS.primary}30`, borderLeftWidth: 3, borderLeftColor: COLORS.primary }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                          ...
                          <Text style={{ color: COLORS.primary, fontSize: FONTS.sizes.xs, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 }}>
                            {FIELD_LABELS[effectiveField] ?? effectiveField} — current text
                          </Text>
                        </View>
                        <Text numberOfLines={3} style={{ color: COLORS.textSecondary, fontSize: FONTS.sizes.sm, lineHeight: 18, fontStyle: 'italic' }}>
                          "{effectiveValue.slice(0, 160)}{effectiveValue.length > 160 ? '…' : ''}"
                        </Text>
                      </View>
                    ) : (
                      <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: COLORS.border, alignItems: 'center', gap: 6 }}>
                        ...
                        <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.sm, textAlign: 'center' }}>
                          {effectiveField ? 'This field is empty. Add some text first.' : 'Pick a field above to rewrite it.'}
                        </Text>
                      </View>
                    )}

                    {/* Style buttons for field rewrite */}
                    <View style={{ gap: SPACING.sm }}>
                      {AI_REWRITE_OPTIONS.map(opt => {
                        ...
                        return (
                          <TouchableOpacity
                            key={opt.id}
                            onPress={() => !disabled && handleRewriteField(opt.id)}
                            disabled={disabled}
                ...
                {/* For bullet layouts — also allow title rewrite */}
                {isBulletLayout && hasBullets && availableFields.includes('title') && (
                    ...
                    <View style={{ flexDirection: 'row', gap: SPACING.sm, flexWrap: 'wrap' }}>
                      {AI_REWRITE_OPTIONS.map(opt => {
                        ...
                        return (
                          <TouchableOpacity
                            key={opt.id}
                            onPress={() => !disabled && onRewriteField('title', opt.id)}
                            disabled={disabled}
                            activeOpacity={0.7}
                            style={{ flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: `${opt.gradient[0]}15`, borderRadius: RADIUS.full, paddingHorizontal: 12, paddingVertical: 7, borderWidth: 1, borderColor: `${opt.gradient[0]}30`, opacity: disabled ? 0.4 : 1 }}
                          >
                            <Ionicons name={opt.icon as any} size={12} color={opt.gradient[0]} />
              ...
              <>
                <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: COLORS.border, padding: SPACING.md }}>
                  ...
                  <TextInput
                    ...
                    multiline
                    numberOfLines={4}
                    style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.sm, minHeight: 90, textAlignVertical: 'top', lineHeight: 20 }}
                  />
                  {genPrompt.length > 0 && (
                ...
                <View>
                  ...
                  <View style={{ flexDirection: 'row', gap: SPACING.sm }}>
                    {(['after', 'before'] as const).map(pos => (
                      <Pressable
                        key={pos}
                        onPress={() => setInsertAfter(pos)}
                        style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 10, borderRadius: RADIUS.lg, backgroundColor: insertAfter === pos ? `${COLORS.primary}18` : COLORS.backgroundElevated, borderWidth: 1.5, borderColor: insertAfter === pos ? COLORS.primary : COLORS.border }}
                      >
                        ...
                        <Text style={{ color: insertAfter === pos ? COLORS.primary : COLORS.textSecondary, fontSize: FONTS.sizes.xs, fontWeight: '700' }}>
                          {pos === 'after' ? `After slide ${currentSlideIndex + 1}` : `Before slide ${currentSlideIndex + 1}`}
                        </Text>
                      </Pressable>
                    ))}
                  </View>
                ...
                <TouchableOpacity
                  ...
                  disabled={genPrompt.trim().length < 5 || !canAffordGenerate}
                  activeOpacity={0.75}
                  style={{ opacity: genPrompt.trim().length < 5 || !canAffordGenerate ? 0.45 : 1 }}
                >
                  <LinearGradient
                    colors={['#FF6584','#F093FB']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                    style={{ borderRadius: RADIUS.full, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, ...SHADOWS.medium }}
                  >
                ...
                {!canAffordGenerate && (
                  <View style={{ backgroundColor: `${COLORS.error}12`, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: `${COLORS.error}30`, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <Ionicons name="alert-circle-outline" size={16} color={COLORS.error} />
                    <Text style={{ color: COLORS.error, fontSize: FONTS.sizes.xs, flex: 1 }}>
                      You need {EDITOR_CREDIT_COSTS.ai_generate} credits. You have {balance}.
                    </Text>
                  </View>
                )}
              </>
              ...
              <>
                {!currentSlide ? (
                  <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: COLORS.border, alignItems: 'center', gap: 8 }}>
                    ...
                    <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.sm, textAlign: 'center' }}>
                      No slide selected. Navigate to a slide first.
                    </Text>
                  </View>
                ) : (
                  <>
                    <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: COLORS.border }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        ...
                        <Text style={{ color: COLORS.primary, fontSize: FONTS.sizes.sm, fontWeight: '700' }}>
                          Slide {currentSlideIndex + 1} — {LAYOUT_LABELS[currentSlide.layout] ?? 'Slide'}
                        </Text>
                      </View>
                    ...
                    <TouchableOpacity
                      onPress={onGenerateSpeakerNotes}
                      disabled={!canAffordNotes}
                      activeOpacity={0.75}
                      style={{ opacity: canAffordNotes ? 1 : 0.45 }}
                    >
                      <LinearGradient
                        colors={['#43E97B','#38F9D7']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                        style={{ borderRadius: RADIUS.full, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, ...SHADOWS.medium }}
                      >
                        <Ionicons name="reader" size={18} color="#FFF" />
                        <Text style={{ color: '#FFF', fontSize: FONTS.sizes.base, fontWeight: '800' }}>
                          {currentSlide.speakerNotes ? 'Regenerate Notes' : 'Generate Notes'}
                        </Text>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: RADIUS.full, paddingHorizontal: 8, paddingVertical: 3 }}>
                          <Ionicons name="flash" size={10} color="#FFF" />
                    ...
                    {!canAffordNotes && (
                      <Text style={{ color: COLORS.error, fontSize: FONTS.sizes.xs, textAlign: 'center' }}>
                        Need {EDITOR_CREDIT_COSTS.ai_notes} credit — you have {balance}.
                      </Text>
                    )}
                  </>
                )}
              ...
              <>
                <View style={{ backgroundColor: COLORS.backgroundElevated, borderRadius: RADIUS.lg, padding: SPACING.md, borderWidth: 1, borderColor: COLORS.border }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: SPACING.sm }}>
                    ...
                    <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.sm, fontWeight: '700' }}>
                      Current: <Text style={{ color: COLORS.primary }}>{LAYOUT_LABELS[currentSlide?.layout ?? ''] ?? 'Unknown'}</Text>
                    </Text>
                  </View>
                  <Text style={{ color: COLORS.textMuted, fontSize: FONTS.sizes.xs, lineHeight: 18 }}>
                    AI analyzes your slide content and recommends the best layout from 11 options.
                  </Text>
                </View>
                ...
                {layoutSuggestion && (
                  <Animated.View entering={FadeInDown.duration(300)} style={{ backgroundColor: `${COLORS.warning}12`, borderRadius: RADIUS.xl, padding: SPACING.md, borderWidth: 1, borderColor: `${COLORS.warning}30` }}>
                    ...
                    <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.base, fontWeight: '700', marginBottom: 4 }}>
                      Switch to: {LAYOUT_LABELS[layoutSuggestion.suggestedLayout] ?? layoutSuggestion.suggestedLayout}
                    </Text>
                ...
                <TouchableOpacity onPress={onSuggestLayout} activeOpacity={0.75}>
                  <LinearGradient
                    colors={['#FFA726','#FF7043']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
                    style={{ borderRadius: RADIUS.full, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, ...SHADOWS.medium }}
                  >
                    <Ionicons name="bulb" size={18} color="#FFF" />
                    <Text style={{ color: '#FFF', fontSize: FONTS.sizes.base, fontWeight: '800' }}>
                      {layoutSuggestion ? 'Analyze Again' : 'Analyze Layout'}
                    </Text>
