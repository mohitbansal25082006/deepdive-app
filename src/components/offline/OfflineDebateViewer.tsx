  ...
  return (
    <View>
      ...
      {session.moderator?.balancedVerdict ? (
        <View style={{
          backgroundColor: cardBg, borderRadius: RADIUS.xl,
...
function PerspectiveCard({ p }: { p: DebatePerspective }) {
  ...
  const cardBg = isLight ? '#FFFFFF' : COLORS.backgroundCard;
  const subBg = isLight ? 'rgba(0,0,0,0.03)' : COLORS.backgroundElevated;
    ...
    <View style={{
      ...
      <TouchableOpacity onPress={() => setExpanded(v => !v)} activeOpacity={0.8}
        ...
        <View style={{
          ...
          backgroundColor: `${p.color}18`, alignItems: 'center', justifyContent: 'center',
          borderWidth: 1, borderColor: `${p.color}30`, flexShrink: 0,
        }}>
        ...
        <View style={{
          backgroundColor: `${sc}18`, borderRadius: RADIUS.full,
          paddingHorizontal: 10, paddingVertical: 4,
          borderWidth: 1, borderColor: `${sc}35`, marginRight: 4,
        }}>
        ...
        <View style={{ paddingHorizontal: SPACING.lg, paddingBottom: SPACING.lg }}>
          ...
          {p.arguments.map((arg, i) => (
            <View key={arg.id ?? i} style={{
              ...
              <Text style={{ color: COLORS.textPrimary, fontSize: FONTS.sizes.sm, fontWeight: '700', marginBottom: 4 }}>
                {arg.point}
              </Text>
              ...
              <View style={{
                ...
                backgroundColor: `${p.color}15`, borderRadius: RADIUS.full,
                paddingHorizontal: 8, paddingVertical: 2,
              }}>
                <Text style={{ color: p.color, fontSize: 9, fontWeight: '700', textTransform: 'uppercase' }}>
                  {arg.strength}
                </Text>
              </View>
            </View>
          ...
          {p.keyQuote ? (
            <View style={{
              borderWidth: 1, borderColor: `${p.color}30`,
              borderRadius: RADIUS.lg, backgroundColor: `${p.color}06`,
              padding: SPACING.md, marginTop: SPACING.sm,
              flexDirection: 'row', gap: 8,
            }}>
              <Text style={{ color: p.color, fontSize: 24, lineHeight: 0.8, flexShrink: 0, marginTop: 4 }}>"</Text>
              <Text style={{ color: COLORS.textSecondary, fontSize: FONTS.sizes.sm, lineHeight: 20, flex: 1, fontStyle: 'italic' }}>
                {p.keyQuote}
              </Text>
            </View>
          ) : null}
    ...
    <View>
      ...
      <LinearGradient colors={gradientColors} style={{
        ...
        <Text style={{
          ...
          fontWeight: '600', lineHeight: 24, fontStyle: 'italic',
        }}>
      ...
      <View style={{ flexDirection: 'row', gap: SPACING.sm, marginBottom: SPACING.md }}>
        <View style={{
          flex: 1, backgroundColor: `${COLORS.success}08`,
          borderRadius: RADIUS.xl, padding: SPACING.md,
          borderWidth: 1, borderColor: `${COLORS.success}25`,
        }}>
          <Text style={{ color: COLORS.success, fontSize: FONTS.sizes.xs, fontWeight: '700', marginBottom: SPACING.sm }}>
            ↑ Arguments For
          </Text>
        ...
        <View style={{
          flex: 1, backgroundColor: `${COLORS.error}08`,
          borderRadius: RADIUS.xl, padding: SPACING.md,
          borderWidth: 1, borderColor: `${COLORS.error}25`,
        }}>
          <Text style={{ color: COLORS.error, fontSize: FONTS.sizes.xs, fontWeight: '700', marginBottom: SPACING.sm }}>
            ↓ Arguments Against
          </Text>
        ...
        <View style={{
          ...
          {moderator.consensusPoints.map((c, i) => (
            <View key={i} style={{
              ...
              padding: SPACING.sm, backgroundColor: `${COLORS.success}08`,
      ...
      {moderator.keyTensions.length > 0 && (
        <View style={{
          ...
          {moderator.keyTensions.map((t, i) => (
            <View key={i} style={{
              flexDirection: 'row', gap: 8, alignItems: 'flex-start',
              padding: SPACING.sm, backgroundColor: `${COLORS.warning}08`,
              borderRadius: RADIUS.md, marginBottom: 6,
      ...
      {/* Neutral conclusion */}
      <View style={{
        backgroundColor: `${COLORS.info}08`, borderRadius: RADIUS.xl,
        padding: SPACING.lg, borderWidth: 1,
        borderColor: `${COLORS.info}25`, borderLeftWidth: 4, borderLeftColor: COLORS.info,
      }}>
        <Text style={{
          color: COLORS.info, fontSize: FONTS.sizes.xs, fontWeight: '700',
          letterSpacing: 0.7, textTransform: 'uppercase', marginBottom: SPACING.sm,
        }}>
          Neutral Conclusion
        </Text>
...
export function OfflineDebateViewer({ session, entry, onClose, onExport, exporting }: OfflineDebateViewerProps) {
  ...
  const backBtnBg = isLight ? 'rgba(0,0,0,0.04)' : COLORS.backgroundElevated;
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.background }}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + SPACING.sm, backgroundColor: headerBg }]}>
        ...
        <View style={[styles.iconContainer, { backgroundColor: `${COLORS.secondary}18` }]}>
          <Ionicons name="chatbox-ellipses-outline" size={15} color={COLORS.secondary} />
        </View>
        ...
        <TouchableOpacity onPress={onExport} disabled={exporting}
          ...
          {exporting ? <ActivityIndicator size="small" color={COLORS.secondary} /> :
            <Ionicons name="document-text-outline" size={16} color={COLORS.secondary} />}
        </TouchableOpacity>

        <TouchableOpacity onPress={handleCopy} disabled={copying}
          style={[styles.copyBtn, { backgroundColor: backBtnBg }]}>
          {copying ? <ActivityIndicator size="small" color={COLORS.textMuted} /> :
            <Ionicons name="copy-outline" size={16} color={COLORS.textMuted} />}
        </TouchableOpacity>
      </View>

      {/* Stance bar */}
      <View style={[styles.stanceBar, { borderBottomColor: COLORS.border }]}>
        <View style={styles.stanceTrack}>
          {forCount > 0 && <View style={{ flex: forCount, backgroundColor: COLORS.success }} />}
          {(session.perspectives.length - forCount - againstCount) > 0 &&
            <View style={{ flex: session.perspectives.length - forCount - againstCount, backgroundColor: COLORS.textMuted }} />}
        ...
        {TABS.map(tab => {
          ...
          return (
            <TouchableOpacity key={tab.id} onPress={() => !isDisabled && setActiveTab(tab.id)} activeOpacity={0.8}
              ...
              <Ionicons name={tab.icon as any} size={17} color={isActive ? COLORS.primary : COLORS.textMuted} />
              <Text style={{
      ...
      {/* Content */}
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: SPACING.xl, paddingBottom: 80 }}
