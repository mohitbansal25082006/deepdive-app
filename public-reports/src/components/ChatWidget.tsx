...
// Public-Reports — Part 55.9 Fully Themed Embedded AI Chat Widget
//
...
export default function ChatWidget({
  ...
  // ── Mount: check if visitor already hit the limit ────────────────────────
  useEffect(() => {
    let cancelled = false;
  ...
  if (!isOpen) {
        ...
        >
          <div className="px-5 py-4 flex items-center gap-4">
            ...
            <button
              ...
              className="flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold transition-all duration-300 hover:scale-105 hover:shadow-lg"
              style={{ 
                background: 'linear-gradient(135deg, var(--theme-gradient-primary-1), var(--theme-gradient-primary-2))',
                color: '#FFFFFF',
                boxShadow: '0 2px 12px var(--theme-primary)',
    ...
    return (
      ...
      >
        <button
          onClick={() => setIsOpen(true)}
          className="w-full px-5 py-4 flex items-center gap-4 text-left transition-all duration-300 hover:bg-white/[0.02]"
        >
          <div
            className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 hover:scale-105"
            style={{
              background: 'linear-gradient(135deg, var(--theme-gradient-primary-1), var(--theme-gradient-primary-2))',
              boxShadow: '0 0 24px var(--theme-primary)',
            }}
          >
        ...
        <div className="px-5 pb-4 flex flex-wrap gap-2" style={{ borderTop: '1px solid var(--theme-border)' }}>
          <p className="w-full text-xs font-semibold pt-3 mb-1" style={{ color: 'var(--theme-text-muted)' }}>
            Try asking:
          </p>
          {suggestions.map(q => (
            <button
              ...
              className="px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-300 hover:scale-105 hover:shadow-md"
              style={{ 
  ...
  return (
    <div
      className="rounded-2xl overflow-hidden flex flex-col animate-fade-in transition-all duration-300 shadow-xl"
      style={{
        background: 'var(--theme-background-card)',
        border: '2px solid var(--theme-primary)',
        height: '520px',
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold" style={{ color: 'var(--theme-text-primary)' }}>AI Research Assistant</p>
          <p className="text-xs" style={{ color: 'var(--theme-text-secondary)' }}>RAG-powered · answers from this report</p>
        </div>
            ...
            <div className="flex gap-1">
              {Array.from({ length: questionsMax }).map((_, i) => (
                <div
                  ...
                  style={{ 
                    background: i < questionsUsed 
                      ? 'var(--theme-text-muted)' 
                      : 'var(--theme-primary)',
                    boxShadow: i < questionsUsed ? 'none' : '0 0 8px var(--theme-primary)',
                  }}
                />
              ))}
            </div>
        ...
        <button
          ...
          className="w-7 h-7 rounded-full flex items-center justify-center transition-all duration-300 hover:bg-white/10 hover:scale-105 flex-shrink-0"
          style={{ border: '1px solid var(--theme-border)' }}
          aria-label="Close"
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
               stroke="var(--theme-text-muted)" strokeWidth="2.5">
            ...
            <div className="flex flex-col gap-2 w-full max-w-xs">
              {suggestions.map(q => (
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl text-xs font-medium text-left transition-all duration-300 hover:scale-105 hover:shadow-md"
                  style={{ 
                    background: 'var(--theme-background-elevated)',
                    border: '1px solid var(--theme-border)',
                    color: 'var(--theme-text-secondary)',
        ...
        <div className="flex-shrink-0 p-3" style={{ borderTop: '1px solid var(--theme-border)' }}>
              ...
              <div
                ...
                style={{
                  ...
                  background: questionsRemaining <= 1
                    ? 'linear-gradient(90deg, var(--theme-warning), var(--theme-error))'
          ...
          <form onSubmit={handleSubmit} className="flex items-end gap-2">
            <textarea
              ...
              disabled={isLoading}
              rows={1}
              className="flex-1 resize-none rounded-xl px-4 py-2.5 text-sm leading-relaxed outline-none transition-all duration-300 focus:shadow-lg"
              style={{
                background: 'var(--theme-background-elevated)',
                border: '1px solid var(--theme-border)',
              ...
              onFocus={e => {
                ...
                e.currentTarget.style.boxShadow = '0 0 0 3px var(--theme-primary)';
              }}
              onBlur={e => {
            ...
            <button
              ...
              className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 hover:scale-105 hover:shadow-lg disabled:opacity-40 disabled:cursor-not-allowed"
              style={{
                background: inputValue.trim() && !isLoading
                  ? 'linear-gradient(135deg, var(--theme-gradient-primary-1), var(--theme-gradient-primary-2))'
                  : 'var(--theme-background-elevated)',
                border: '1px solid var(--theme-border)',
                boxShadow: inputValue.trim() && !isLoading ? '0 2px 12px var(--theme-primary)' : 'none',
              }}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                   stroke={inputValue.trim() && !isLoading ? 'white' : 'var(--theme-text-muted)'}
          ...
          <p className="text-center text-xs mt-2" style={{ color: 'var(--theme-text-muted)' }}>
